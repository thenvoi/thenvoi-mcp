...
class TestGetAgentMe:
    ...
    def test_returns_agent_profile(self, mock_ctx, mock_agent_api_identity):
        """Test successful retrieval of agent profile."""
        agent = factory.agent_me(
            name="Weather Agent", description="Provides weather info"
        )
        mock_agent_api_identity.get_agent_me.return_value = factory.response(agent)

        result = get_agent_me(mock_ctx)

        mock_agent_api_identity.get_agent_me.assert_called_once()
        parsed = json.loads(result)
        assert parsed["data"]["name"] == "Weather Agent"
        assert parsed["data"]["description"] == "Provides weather info"

    def test_includes_agent_id_and_timestamps(self, mock_ctx, mock_agent_api_identity):
...
class TestListAgentPeers:
    ...
    def test_returns_list_of_peers(self, mock_ctx, mock_agent_api_peers):
        ...
        mock_agent_api_peers.list_agent_peers.return_value = factory.list_response(
            peers
        )

        result = list_agent_peers(mock_ctx)

        mock_agent_api_peers.list_agent_peers.assert_called_once_with(
            not_in_chat=None,
            page=None,
            page_size=None,
        )
    ...
    def test_filters_by_not_in_chat(self, mock_ctx, mock_agent_api_peers):
    ...
    def test_pagination_parameters(self, mock_ctx, mock_agent_api_peers):
    ...
    def test_empty_peer_list(self, mock_ctx, mock_agent_api_peers):
