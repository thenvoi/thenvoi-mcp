...
class TestGetAgentChatContext:
    ...
    def test_returns_context_messages(self, mock_ctx, mock_agent_api_context):
        ...
        mock_agent_api_context.get_agent_chat_context.return_value = (
            factory.list_response(messages)
        )

        result = get_agent_chat_context(mock_ctx, chat_id=chat_id)

        mock_agent_api_context.get_agent_chat_context.assert_called_once_with(
            chat_id=chat_id,
            page=None,
            page_size=None,
        )
    ...
    def test_pagination_parameters(self, mock_ctx, mock_agent_api_context):
    ...
    def test_empty_context(self, mock_ctx, mock_agent_api_context):
...
class TestCreateAgentChatMessage:
    ...
    def test_creates_message_with_recipients(
        self, mock_ctx, mock_agent_api_participants, mock_agent_api_messages
    ):
        ...
        mock_agent_api_participants.list_agent_chat_participants.return_value = (
            factory.list_response([participant])
        )
        message = factory.chat_message(id="msg-789", content=content)
        mock_agent_api_messages.create_agent_chat_message.return_value = (
            factory.response(message)
        )
        ...
        mock_agent_api_participants.list_agent_chat_participants.assert_called_once_with(
            chat_id=chat_id
        )
        mock_agent_api_messages.create_agent_chat_message.assert_called_once()
        parsed = json.loads(result)
        assert parsed["data"]["id"] == "msg-789"

    def test_creates_message_with_mentions(self, mock_ctx, mock_agent_api_messages):
    ...
    def test_mentions_takes_precedence_over_recipients(
        self, mock_ctx, mock_agent_api_participants, mock_agent_api_messages
    ):
        ...
        # Should NOT call list_agent_chat_participants when mentions is provided
        mock_agent_api_participants.list_agent_chat_participants.assert_not_called()

    def test_raises_when_no_recipients_or_mentions(self, mock_ctx):
    ...
    def test_raises_on_invalid_mentions_json(self, mock_ctx):
    ...
    def test_raises_when_recipient_not_found(
        self, mock_ctx, mock_agent_api_participants
    ):
    ...
    def test_multiple_recipients(
        ...
        call_args = mock_agent_api_messages.create_agent_chat_message.call_args
        mentions = call_args.kwargs["message"].mentions
        assert len(mentions) == 2

    def test_recipient_matching_is_case_insensitive(
        self, mock_ctx, mock_agent_api_participants, mock_agent_api_messages
    ):
