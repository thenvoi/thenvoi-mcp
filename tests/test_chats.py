...
class TestListAgentChats:
    ...
    def test_returns_list_of_chats(self, mock_ctx, mock_agent_api_chats):
        """Test successful retrieval of chat list."""
        chats = [
            factory.chat_room(title="Project Alpha"),
            factory.chat_room(title="Support Chat"),
        ]
        mock_agent_api_chats.list_agent_chats.return_value = factory.list_response(
            chats
        )

        result = list_agent_chats(mock_ctx)

        mock_agent_api_chats.list_agent_chats.assert_called_once_with(
            page=None,
            page_size=None,
        )
    ...
    def test_pagination_parameters(self, mock_ctx, mock_agent_api_chats):
    ...
    def test_empty_chat_list(self, mock_ctx, mock_agent_api_chats):
...
class TestGetAgentChat:
    ...
    def test_returns_chat_details(self, mock_ctx, mock_agent_api_chats):
        """Test successful retrieval of chat details."""
        chat_id = "chat-456"
        chat = factory.chat_room(id=chat_id, title="Team Discussion")
        mock_agent_api_chats.get_agent_chat.return_value = factory.response(chat)

        result = get_agent_chat(mock_ctx, chat_id=chat_id)

        mock_agent_api_chats.get_agent_chat.assert_called_once_with(id=chat_id)
        parsed = json.loads(result)
        assert parsed["data"]["id"] == chat_id
        assert parsed["data"]["title"] == "Team Discussion"

    def test_includes_timestamps(self, mock_ctx, mock_agent_api_chats):
...
class TestCreateAgentChat:
    ...
    def test_creates_chat_without_task(self, mock_ctx, mock_agent_api_chats):
        """Test creating a chat room without associated task."""
        chat_id = "new-chat-123"
        chat = factory.chat_room(id=chat_id)
        mock_agent_api_chats.create_agent_chat.return_value = factory.response(chat)

        result = create_agent_chat(mock_ctx)

        mock_agent_api_chats.create_agent_chat.assert_called_once()
        call_args = mock_agent_api_chats.create_agent_chat.call_args
        assert call_args.kwargs["chat"].task_id is None
        parsed = json.loads(result)
        assert parsed["data"]["id"] == chat_id

    def test_creates_chat_with_task_id(self, mock_ctx, mock_agent_api_chats):
